...
let toDos = []; // array 초기화 방지하기 우해 업데이트가 가능한 let 으로 변경.

function saveToDos() { //save to localStorage
    localStorage.setItem("todos",JSON.stringify(toDos)); //JSON.stringfify() 쓰지않고 toDos만 써서 오류남.
    //JSON.stringify() value,object 를 string 으로 변경해줌
...
function deleteToDo(event) {
    ...
    //li 는 DOM에 직접 접근하여 ID 를 주었으므로 string 이다.
    li.remove();
    toDos = toDos.filter((toDo) => toDo.id !== parseInt(li.id));
    saveToDos();
}
...
// array에서 item 을 제거하는 것은 사실 해당 item을 제외한 array를 복제하는 것.
